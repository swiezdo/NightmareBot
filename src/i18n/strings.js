...
export const strings = {
  en: {
    ...
    confirm_saved: 'The JSON file has been written. You can run `/setup-waves` or `/edit-waves` again.',
    ...
    edit_tsushima_missing: '**waves/tsushima.json** was not found. Run `/setup-waves` first or create the file.',
    edit_tsushima_invalid: 'Cannot load **waves/tsushima.json** (invalid JSON or empty array). Fix the file and try again.',
    week_not_in_rotation:
      'This **week** is not in the current rotation JSON. Update `week` in the file or run `/setup-waves` to pick a valid week.',
  ...
  ru: {
    ...
    confirm_saved: 'Файл записан. Можно снова вызвать `/setup-waves` или `/edit-waves`.',
    ...
    edit_tsushima_missing: 'Файл **waves/tsushima.json** не найден. Сначала выполните `/setup-waves` или создайте файл.',
    edit_tsushima_invalid: 'Не удалось прочитать **waves/tsushima.json** (битый JSON или пустой массив). Исправьте файл и повторите.',
    week_not_in_rotation:
      'Эта **неделя** не найдена в текущем rotation JSON. Обновите `week` в файле или запустите `/setup-waves`, чтобы выбрать актуальную неделю.',
